...
> 建议：给标识符命名时，争取做到见名知意的效果，方便自己和他人的阅读，或者打上单行注释
...
| **数据类型**        | **占用空间**                                    | 取值范围         | 取值范围（位数） |
| ------------------- | ----------------------------------------------- | ---------------- | ---------------- |
| short(短整型)       | 2字节                                           | (-2^15 ~ 2^15-1) | 10^4             |
| int(整型)           | 4字节                                           | (-2^31 ~ 2^31-1) | 10^9             |
| long(长整形)        | Windows为4字节，Linux为4字节(32位)，8字节(64位) | (-2^31 ~ 2^31-1) | 10^9             |
| long long(长长整形) | 8字节                                           | (-2^63 ~ 2^63-1) | 10^19            |
...
在 C++ 中，像 `3.14` 这样的浮点字面量默认是 `double` 类型，而不是 `float`。
...
编译器会将 `3.14`（double）转换为 `float` 赋值给 `f1`。这种隐式转换通常是可以的，但可能会引发两种问题：

1. **精度丢失警告**：很多编译器会发出警告（例如 `warning: conversion from 'double' to 'float' may change value`），因为 `double` 的精度比 `float` 高，转换可能丢失一些细节。
2. **函数重载歧义**：如果存在重载函数，比如 `void func(float)` 和 `void func(double)`，调用 `func(3.14)` 会匹配 `double` 版本，这可能不是你的本意。

加上 `f` 后缀（或 `F`）可以明确指定字面量为 `float` 类型：

```cpp
float f1 = 3.14f;   // 直接使用 float 字面量
```

这样就不会发生类型转换，也避免了上述警告和重载问题。

所以，**从语法上讲，`f` 并不是必须的**——代码仍然能编译运行。但从**代码清晰度和避免潜在问题**的角度来看，**强烈建议加上 `f` 后缀**，以表明你确实希望使用 `float` 类型并让编译器直接生成单精度浮点数。

类似地，如果想要使用 `long double` 字面量，可以用 `l` 或 `L` 后缀，例如 `3.14L`。

3. **输出格式**  
   `cout` 默认会根据数值大小自动选择输出格式（定点或科学计数法）。例如 `300` 输出 `"300"`，`0.03` 输出 `"0.03"`。若希望统一格式，可使用 `fixed` 或 `scientific` 
   
   ```cpp
   cout << fixed << f3;        // 强制定点表示
   cout << scientific << f2;    // 强制科学计数法
   ```
...
graph TB
	A([开始])-->B{条件判断}
	B{条件判断}-->|Ture|C[执行语句1]
	B{条件判断}-->|Flase|D[执行语句2]
	C[执行语句1] & D[执行语句2]-->E([结束])
```
...
graph TB
	...
	G{······}-->|Flase|I{条件判断}
	I{条件判断}-->|Flase|J[执行语句i]
	I{条件判断}-->|Ture|K[执行语句i+1]
	J[执行语句i] & K[执行语句i+1]-->E[剩余执行语句]
